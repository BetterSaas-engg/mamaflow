...
# date at all, so without this the extractor cannot see the event. The
# ICS-text parsing itself is provider-neutral and lives in email_body.py;
# this module only walks Gmail's JSON payload shape.
...
def _extract_calendar_summary(payload: dict) -> str:
    ...
    return calendar_summary_from_ics(_extract_calendar_raw(payload))


def _compose_body(payload: dict) -> str:
    """The text the pipeline treats as the email body (see email_body)."""
