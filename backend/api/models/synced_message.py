...
class SyncedMessage(TimestampMixin, Base):
    ...
    count, so zero-event mail is never re-extracted.

    Since D39 a FAILED extraction also writes a marker — carrying `attempts`
    and `failure_kind` — so retries are bounded instead of running hourly
    forever. Such a row suppresses the message only once we've given up on it
    (see `items.existing_message_ids`); an in-flight retry stays eligible.
    """
