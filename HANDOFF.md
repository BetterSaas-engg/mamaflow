...
| B | Android sign-in + Firebase/FCM sender + APNs (D22/D26/D27) | **Sign-in DONE 2026-07-04 (code)** — D30: Android **reuses the iOS OAuth client** (no separate Android client / SHA-1 needed for auth). Registered the `flutter_web_auth_2` `CallbackActivity` with the reversed-iOS scheme in `frontend/android/app/src/main/AndroidManifest.xml`; app takes the same `--dart-define=GOOGLE_IOS_CLIENT_ID`. **USER: run on an Android emulator/device to verify** — `flutter run -d <android> --dart-define=API_BASE_URL=https://mamaflow-production.up.railway.app --dart-define=GOOGLE_IOS_CLIENT_ID=<ios client id>` (emulator base URL `http://10.0.2.2:8000` only if pointing at a local backend). Requires the Android SDK/emulator set up (`flutter doctor`). **Still gated (push):** Firebase project + Android OAuth client (SHA-1 `8B:E8:14:1C:8C:E8:73:1F:EB:2D:52:1A:8B:EF:4A:67:7C:D1:B8:B9`) + APNs key → then the FCM sender (D27). |
